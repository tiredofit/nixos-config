...

{
  options = {
    services = {
     docker_container_manager_apps = with types; {
        enable = mkEnableOption "Start and stop containers in non '_system' folders.";
      };
      docker_container_manager_system = with types; {
        enable = mkEnableOption "Start and stop containers in '_system' folder";
      };
    };
  };
  config =
    let
      ...
      '';
    in
    mkIf apps_cfg.enable {
      systemd.services.docker_container_manager_apps = {
        Unit = {
          Description = "Start and stop containers not in '_system' folder";
          After = "docker.service";
        };
        Service = {
          Type = "oneshot";
          ExecStart = "${mkStartScript name} apps";
